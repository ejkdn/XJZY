...
"""
            ...
            cursor.execute("update xjzy set Gold = ?, Stealth_potions = ?, How_much_feed_the_chicks_ate_in_total = ?, Accelerator_card = ?, How_much_corn_is_there_in_total = ?, How_many_corn_seeds_there_were = ? where id = 1", (Gold, Stealth_potions, How_much_feed_the_chicks_ate_in_total, Accelerator_card, How_much_corn_is_there_in_total, How_many_corn_seeds_there_were)) #更改数据
            cursor.close() #关闭游标
            conn.commit() #提交事务
            conn.close() #关闭数据库
            prco("拜拜\n")
